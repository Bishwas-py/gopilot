...
server.tool("fetch_openapi", "Fetch the OpenAPI 3.1 spec from the running Go backend and cache it locally", {
    backendUrl: z
        .string()
...
server.tool("generate_schemas", "Generate TypeScript validation schemas (Valibot or Zod) from an OpenAPI spec", {
    openapiPath: z
        .string()
...
server.tool("generate_remotes", "Generate typed remote functions (SvelteKit .remote.ts or Next.js .server.ts) from an OpenAPI spec", {
    openapiPath: z
        .string()
        .describe("Path to openapi.json"),
    outputDir: z
    ...
    const result = await generateRemotes(openapiPath, outputDir, schemasImportPath, framework);
    return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
    };
});
// Tool: Scaffold a new Go-first project
server.tool("scaffold_project", "Scaffold a new Go-first full-stack project with backend and frontend structure", {
    ...
    const result = await scaffoldProject(projectName, projectDir, framework, goModule);
    return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
    };
});
